...
def setEnvironment(caller):
    ...
    if caller == "GUI":
        ...
        cwd = os.getcwd()
        if osName == "win":
            # full_path = os.path.join(base_path, "FuncParser.exe") // Need to Download FuncParser.exe in Initialize file
...
def parseFile_shallow(srcFileName, caller):
    ...
    astString = ""
    try:
        astOutput = subprocess.check_output(javaCallCommand, stderr=subprocess.STDOUT, shell=True)
        if astOutput:
            astString = str(astOutput, "utf-8")
...
def parseFile_deep(srcFileName, caller):
    ...
    funcList = astString.decode().split(delimiter)
    for func in funcList[1:]:
        functionInstance = function(srcFileName)
