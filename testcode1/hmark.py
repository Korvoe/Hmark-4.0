...
def parseFiles_shallow(f, lang):
    if lang == "c":
        functionInstanceList = pu.parse_c_shallow(f)
...
def parseFiles_deep(f, lang):
    if lang == "c":
        functionInstanceList = pu.parse_c_deep(f)
        return (f, functionInstanceList, lang)
    elif lang == "java":
...
#        self.btnChoose.pack(side=Tkinter.RIGHT)
    ...
    def generate(self):
        ...
        if numFile == 0:
            ...
            hashList = open("result.txt", "a+")
            originalList = open("original.txt", "a+")
            abstractList = open("abstract.txt", "a+")
